...
def classify_sentiment(text: str)
    Classify the sentiment of the given text  # Preamble (write implicit)
...
def generate_sql(entities: list(str), attributes: mapping(str, list[str], relationships: list[tuple[str, str, str])
...
def generate_code(spec: str, tests: list[callable[python[str]]): str
    Write a Python function with the following specifications.  # Preamble
    {spec}                                                      # Input
    let code: python[str] = read {python[str]}
    let counter: int = 0
    try
        label retry
...
let sentiment = classify_sentiment(review)
if sentiment == "negative" do
    let summary = summarize_issue(review)
endif
\end{lstlisting}
...
def answer_with_docs(user_message,
                     documents):
    Q: Answer the user referring to
...
def detect_query(message):
    Q: Is the user asking a query
       in this message "{message}"?
    A: {yes|no}
    if output == "yes" do
        return true
    else
