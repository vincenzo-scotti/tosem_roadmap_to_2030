...
\begin{enumerate*}[label=(\arabic*)]
  \item \emph{system message} (main task instructions, which can also be chatbot directives);
  \item \emph{user message} (user's input to be processed, question, or request);
  \item \emph{agent response} (agent's output or response to user's question or request).
\end{enumerate*}
...
\begin{enumerate*}[label=(\roman*)]
 \item \emph{few-shot learning} (sometimes called in-context learning)~\cite{DBLP:conf/nips/BrownMRSKDNSSAA20},
 \item \emph{Retrieval Augmented Generation} (RAG)~\cite{DBLP:journals/corr/abs-2312-10997}, and
 \item \emph{Chain of Thought} (CoT) \emph{reasoning}~\cite{DBLP:conf/nips/KojimaGRMI22}.
\end{enumerate*}
...
\begin{enumerate*}[label=(\arabic*)]
  \item context size (i.e., the maximum number of consecutive tokens a model can manage) and
...
\begin{enumerate*}[label=(\roman*)]
  \item \emph{interaction-centricity}, 
  \item \emph{explicit context and constraints}, and
  \item \emph{analyzability over expressiveness}.
\end{enumerate*}
...
The survey received 21 responses; after applying the quality control mechanisms described in \Cref{sec:threats-mitigation}, 20 valid responses were retained for analysis.
The following paragraphs report descriptive results for each section of the questionnaire, with figures organised by thematic block.
...
To analyze differences in perceived alignment across the seven roles, we employed a Friedman test to determine whether statistically significant differences exist at the group level, followed by post-hoc pairwise Wilcoxon signed-rank tests comparing the specialized roles against the generic \texttt{contributor} baseline.
